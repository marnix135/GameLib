...
public class Terrain {
    ...
    public void render() {
        ...
        if (mesh.hasTextureCoords()) glEnableVertexAttribArray(1);
        if (mesh.hasNormals()) glEnableVertexAttribArray(2);

        glDrawElements(GL_TRIANGLES, indicesSize, GL_UNSIGNED_INT, 0);

        glDisableVertexAttribArray(0);
        if (mesh.hasTextureCoords()) glDisableVertexAttribArray(1);
        if (mesh.hasNormals()) glDisableVertexAttribArray(2);
        glBindVertexArray(0);
    }
